...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="card card-hover animate-fade" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Product Image Container */}
      <Link href={`/products/${product.id}`} style={{ display: 'block', overflow: 'hidden', borderRadius: 'var(--radius-md)', aspectRatio: '1/1', background: '#f1f5f9', position: 'relative' }}>
        <img
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', marginTop: '16px' }}>
        ...
        <Link href={`/products/${product.id}`} style={{ display: 'block', marginBottom: '8px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '700', color: 'var(--text-primary)', lineBreak: 'anywhere' }}>
            {product.name}
          </h3>
        </Link>

        <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '16px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineBreak: 'anywhere' }}>
          {product.description}
        </p>
