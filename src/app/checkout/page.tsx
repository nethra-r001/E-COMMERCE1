...
import { ShieldCheck, CreditCard, ChevronRight, CheckCircle, ArrowLeft } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to process checkout. Please check your network connection.');
      setLoading(false);
    }
  };
    ...
    return (
      <main className="section" style={{ minHeight: '80vh', display: 'flex', alignItems: 'center' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: '500px' }}>
          <div style={{ display: 'inline-flex', width: '80px', height: '80px', borderRadius: '50%', background: 'var(--success-glow)', color: 'var(--success)', alignItems: 'center', justifyContent: 'center', marginBottom: '24px' }}>
            <CheckCircle size={44} />
          </div>
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: '16px', fontSize: '15px' }}>
            Thank you for your purchase. Your payment was validated successfully, and we have received your order.
          </p>
          <div className="card" style={{ padding: '16px', backgroundColor: 'var(--bg-color)', marginBottom: '32px' }}>
            <span style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'block', textTransform: 'uppercase', fontWeight: '600' }}>
              Your Transaction ID
            </span>
    ...
    <main className="section animate-fade" style={{ minHeight: '80vh' }}>
      <div className="container" style={{ maxWidth: '1000px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', flexWrap: 'wrap', gap: '16px' }}>
          <h1 style={{ fontSize: '32px', fontWeight: '800' }}>Secure Checkout</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '40px' }} className="checkout-layout">
              ...
              <div className="card glass-card">
                ...
                <form onSubmit={handleNextStep}>
                  ...
                  <div className="form-group" style={{ marginBottom: '28px' }}>
                    <label className="form-label" htmlFor="phone">Contact Mobile Phone (10 digits)</label>
                    <input
                      type="tel"
                      id="phone"
                      className="form-input"
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Link href="/cart" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '14px', color: 'var(--text-secondary)' }} className="btn-link">
                      <ArrowLeft size={16} />
          ...
          <div>
            <div className="card glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '180px', overflowY: 'auto', paddingRight: '4px' }}>
                {cart.map((item) => (
                  <div key={item.product.id} style={{ display: 'flex', justifyItems: 'center', gap: '12px' }}>
                    <img 
                    ...
                    <div style={{ flex: 1, fontSize: '13px' }}>
                      <span style={{ fontWeight: '700', color: 'var(--text-primary)', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '150px' }}>
                        {item.product.name}
                      </span>
              ...
              {/* Price Details */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '13px', borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '16px', fontWeight: '800', borderTop: '1px solid var(--border-color)', paddingTop: '12px', marginTop: '4px' }}>
                  <span>Total Due</span>
              ...
              <div style={{ display: 'flex', gap: '10px', background: 'var(--bg-color)', padding: '12px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                <ShieldCheck size={28} style={{ color: 'var(--success)', flexShrink: 0 }} />
                <p style={{ fontSize: '11px', color: 'var(--text-secondary)', margin: '0' }}>
                  Your details are fully protected. All data is sent encrypted over secure SSL gateway protocol.
                </p>
              </div>
            </div>
          </div>
