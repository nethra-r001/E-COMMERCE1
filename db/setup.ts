...
async function setup() {
  ...
  if (usersCount && usersCount.count === 0) {
    ...
    await db.run(
      ...
      ['u-admin', 'Admin User', 'admin@example.com', adminPasswordHash, '9876543210', 'ADMIN']
    );
    await db.run(
      'INSERT INTO users (id, name, email, password, phone, role) VALUES (?, ?, ?, ?, ?, ?)',
      ['u-customer1', 'John Doe', 'user@example.com', userPasswordHash, '1234567890', 'USER']
    );
    await db.run(
      'INSERT INTO users (id, name, email, password, phone, role) VALUES (?, ?, ?, ?, ?, ?)',
      ['u-customer2', 'Jane Smith', 'jane@example.com', userPasswordHash, '9998887776', 'USER']
    );
    console.log('Users seeded: admin@example.com (admin123) and user@example.com (user123) with phone numbers');
  }

  // Check if products exist. If not, seed products.
  const productsCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM products');
  ...
  if (productsCount && productsCount.count === 0) {
    ...
    productsList = [
      {
        ...
        description: 'Premium active noise-cancelling wireless headphones with 40-hour battery life, high-fidelity sound, and ergonomic design.',
        price: 149.99,
        stock: 45,
      ...
      },
      {
        ...
        description: 'Sleek smartwatch featuring real-time health tracking, heart rate monitoring, GPS, and custom watch faces with up to 7 days of battery life.',
        price: 199.99,
        stock: 30,
      ...
      },
      {
        ...
        description: 'Tenkeyless RGB mechanical keyboard with custom brown switches for responsive typing and gaming. Includes premium double-shot PBT keycaps.',
        price: 89.99,
        stock: 75,
      ...
      },
      {
        ...
        description: 'Classic fit bomber jacket made from premium sustainable synthetic leather. Warm, wind-resistant, and perfect for styling any outfit.',
        price: 129.99,
        stock: 15,
      ...
      },
      {
        ...
        description: 'Pack of 3 essential t-shirts made from 100% organic cotton. Soft, breathable, and pre-shrunk for the perfect everyday fit.',
        price: 34.99,
        stock: 120,
      ...
      },
      {
        ...
        description: 'Non-slip natural cork yoga mat with alignment lines. Provides excellent cushioning, grip, and natural antimicrobial properties.',
        price: 49.99,
        stock: 50,
      ...
      },
      {
        ...
        description: 'Double-walled vacuum insulated water bottle. Keeps drinks ice cold for 24 hours or piping hot for 12 hours. Leakproof loop cap.',
        price: 24.99,
        stock: 200,
      ...
      },
      {
        ...
        description: '1.5L glass carafe cold brew maker with airtight lid and ultra-fine stainless steel mesh filter for smooth, sediment-free coffee concentrated extract.',
        price: 39.99,
        stock: 40,
      ...
      },
      {
        ...
        description: 'WiFi-connected tabletop ambient lamp. Select from millions of colors and warm/cool whites using smartphone app or voice control integrations.',
        price: 54.99,
        stock: 65,
      ...
      },
      {
        ...
        description: 'Set of 3 matte white ceramic vases in varied modern geometric shapes. Elegant decor piece for dried or fresh bouquets.',
        price: 29.99,
        stock: 80,
        image_url: 'https://images.unsplash.com/photo-1578500494198-246f612d3b3d?w=600&auto=format&fit=crop&q=80',
        category: 'Home & Living'
      }
    ];
    ...
    for (const p of productsList) {
      await db.run(
        ...
        [p.id, p.name, p.description, p.price, p.stock, p.image_url, p.category]
      );
    }
  ...
  const ordersCount = await db.get<{ count: number }>('SELECT COUNT(*) as count FROM orders');
  if (ordersCount && ordersCount.count === 0 && productsList.length > 0) {
    ...
    const orderStatuses = ['PENDING', 'PROCESSING', 'SHIPPED', 'DELIVERED', 'DELIVERED', 'DELIVERED'];
